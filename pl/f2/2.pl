...
% a) Implemente o predicado ancestor(?X, ?Y) que suceda se X é um antepassado
% de Y.
ancestor(X, Y):-X = Y.
ancestor(X, Y):-parent(P, Y),
                ancestor(X, P).

% b) Implemente o predicado descendant (?X, ?Y) que sucede se X é descendente
% de Y.
descendant(X, Y):-X = Y.
descendant(X, Y):-parent(Y, C),
                  descendant(X, C).
